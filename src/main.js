...
try {
  ...
  const maximumPages       = input.maximumPages       ?? 1;

  const serviceName        = 'LinkedIn Profile Activity';
  const serviceOption1     = 'linkedin-profile-activity';
  const requestSource      = 'LinkedIn_Profile_Activity_AP';
  const boomerangInputUrl  = 'https://PLACEHOLDER.boomerangserver.co.in/webhook/profile-activity';
  const boomerangStatUrl   = 'https://PLACEHOLDER.boomerangserver.co.in/webhook/profile-activity-stats';
  ...
  if (!linkedinProfileUrl.trim()) throw new Error('LinkedIn Profile URL is required!');

  // ──────────────────────────────
  // 2. VALIDATE URL
  // ──────────────────────────────
  const cleanUrl = linkedinProfileUrl.trim();
  const isValid  =
    cleanUrl.startsWith('https://www.linkedin.com/in/') ||
    cleanUrl.startsWith('http://www.linkedin.com/in/')  ||
    cleanUrl.startsWith('https://in.linkedin.com/in/');

  if (!isValid) throw new Error('Invalid LinkedIn profile URL. Must start with https://www.linkedin.com/in/');

  const rowCount   = 1;
  const csvContent = 'LinkedIn Profile URL,maximumPages\n' + `${cleanUrl},${maximumPages}`;
  ...
  while (batchJobs && batchJobs.length > 0) {
          ...
          await fetch(
            'https://frontend.boomerangserver.co.in/webhook/Status_and_output_universal',
            {
              ...
              body   : JSON.stringify({
                ...
                requestStatus     : 'Error',
                driveInputLink,
                boomerangOutputUrl: `https://PLACEHOLDER.boomerangserver.co.in/webhook/profile-activity-output?request_id=${request_id}`,
                batch_number,
                request_unique_id,
                batchFolderId,
                service_option_1  : serviceOption1,
    ...
    for (const result of batchStatusResults) {
      ...
      const boomerangOutputUrl = `https://PLACEHOLDER.boomerangserver.co.in/webhook/profile-activity-output?request_id=${request_id}`;
